...
-- ****************************************************************************************************************/
  ...
  */
  ...
  TYPE gt_xxaqv_attach_docs_type IS
    TABLE OF xxaqv_attach_docs_stg%rowtype INDEX BY BINARY_INTEGER;
     TYPE gt_fnd_lobs_type IS
    TABLE OF fnd_lobs%rowtype INDEX BY BINARY_INTEGER;	

   -- Global Records
   gt_xxaqv_attach_docs_tab     gt_xxaqv_attach_docs_type;
   gt_fnd_lobs_tab              gt_fnd_lobs_type;
...
-- ****************************************************************************************************************/
   ...
   BEGIN
      SELECT COUNT(*)
       ...
       WHERE 1           = 1
         AND entity_name = gv_entity_name;
  ...
  IS
      SELECT fad.seq_num       sequence_num
       ...
       WHERE fdst.media_id   = fd.media_id
         ...
         AND EXISTS ( SELECT 1
                        FROM xxaqv_ap_suppliers_c003_stg apt
                       WHERE apt.segment1    = aps.segment1  )
         AND NOT EXISTS ( SELECT 1
                            FROM xxaqv_attach_docs_stg xads
                           WHERE xads.vendor_number = aps.segment1    
                             AND xads.seq_num       = fad.seq_num
                             AND xads.entity_name   = fad.entity_name);    
    ...
    LOOP
     ...
     BEGIN
          FORALL i IN gt_xxaqv_attach_docs_tab.FIRST..gt_xxaqv_attach_docs_tab.LAST SAVE EXCEPTIONS
            INSERT INTO xxaqv_attach_docs_stg VALUES gt_xxaqv_attach_docs_tab ( i );
    ...
    LOOP
     ...
     BEGIN
          ...
          FORALL j IN gt_xxaqv_attach_docs_tab.first..gt_xxaqv_attach_docs_tab.last SAVE EXCEPTIONS
            INSERT INTO xxaqv_attach_docs_stg VALUES gt_xxaqv_attach_docs_tab ( j );
    ...
    LOOP
     ...
     BEGIN
          FORALL z IN gt_xxaqv_attach_docs_tab.first..gt_xxaqv_attach_docs_tab.last SAVE EXCEPTIONS
            INSERT INTO xxaqv_attach_docs_stg VALUES gt_xxaqv_attach_docs_tab ( z );
    ...
    LOOP
     ...
     BEGIN
          FORALL m IN gt_xxaqv_attach_docs_tab.first..gt_xxaqv_attach_docs_tab.last SAVE EXCEPTIONS
            INSERT INTO xxaqv_attach_docs_stg VALUES gt_xxaqv_attach_docs_tab ( m );
         ...
         EXCEPTION
            ...
            WHEN OTHERS 
            THEN
               print_debug('EXTRACT_DATA:File Unexpected Error: ' || sqlerrm, gv_log_type );
               x_retcode   := 1;
               x_err_msg   := gv_module
                           ||':'
 ...
 BEGIN
           ...
           --LOOP
              ...
              UPDATE xxaqv_attach_docs_stg 
                 SET file_data = ( SELECT file_data
                                          FROM fnd_lobs@xxaqv_conv_cmn_dblink fl
                                         WHERE x_file_id = fl.file_id       );
              -- WHERE rowid     = r_cur_file_data.rowid ; 
 ...
 -- This Cursor is used to retrieve information about Short Text Attachments. --
      ...
      SELECT fad.seq_num             sequence_num
       ...
       WHERE fdst.media_id         = fd.media_id
         ...
         AND EXISTS (SELECT 1 
                       FROM xxaqv_ap_supp_site_c003_stg asst
                      WHERE asst.vendor_num       = aps.segment1
                        AND assa.vendor_site_code = asst.vendor_site_code  )
         AND NOT EXISTS ( SELECT 1
                            FROM xxaqv_attach_docs_stg xads
                           WHERE xads.vendor_number    = aps.segment1      
                             AND xads.vendor_site_code = assa.vendor_site_code
                             AND xads.seq_num          = fad.seq_num
                             AND xads.entity_name      = fad.entity_name);						
    ...
    LOOP
     ...
     BEGIN
          FORALL k IN gt_xxaqv_attach_docs_tab.first..gt_xxaqv_attach_docs_tab.last SAVE EXCEPTIONS
            INSERT INTO xxaqv_attach_docs_stg VALUES gt_xxaqv_attach_docs_tab ( k );
           ...
           LOOP
              ...
              UPDATE xxaqv_attach_docs_stg xads
                 ...
                 SET xads.file_data = ( SELECT file_data
                                          FROM fnd_lobs@xxaqv_conv_cmn_dblink fl
                                         WHERE fl.file_id = xads.x_file_id  )
               WHERE xads.entity_name     = gv_entity_name
 ...
 -- This Cursor is used to retrieve information about Short Text Attachments. --
      ...
      IS
       ...
       WHERE aias.invoice_id     = fad.pk1_value
         ...
         AND EXISTS ( SELECT 1
                        FROM xxaqv_ap_inv_header_stg     aiat                           
                       WHERE aiat.invoice_num      = aias.invoice_num
                         and aiat.vendor_num       = aps.segment1
                         and aiat.vendor_site_code = assa.vendor_site_code)
      AND NOT EXISTS (SELECT 1 
                        FROM xxaqv_attach_docs_stg xads
                       WHERE xads.invoice_number   = aias.invoice_num
                         AND xads.vendor_site_code = assa.vendor_site_code
                         AND xads.vendor_number    = aps.segment1); 
    ...
    LOOP
     ...
     BEGIN
          FORALL z IN gt_xxaqv_attach_docs_tab.first..gt_xxaqv_attach_docs_tab.last SAVE EXCEPTIONS
            ...
            print_debug(gv_module
                        ||':'
                        ||'EXTRACT_DATA: XXAQV_ATTACH_DOCS_STG:WEB_PAGE Records loaded sucessfully: ' 
                        || SQL%rowcount);         
         COMMIT;
         ...
         EXCEPTION
            ...
            THEN
              ...
              print_debug(gv_module
                         ||':'
                         ||'EXTRACT_DATA: XXAQV_ATTACH_DOCS_STG:WEB_PAGE Number of failures: ' 
                         || ln_error_count,gv_log_type); 
               FOR i IN 1..ln_error_count 
               LOOP 
               print_debug(gv_module
                           ||':'
                           ||'EXTRACT_DATA: XXAQV_ATTACH_DOCS_STG:WEB_PAGE Error: ' 
                           || i 
                           || 'Array Index: '
                           || SQL%bulk_exceptions(i).error_index 
                           || 'Message: ' 
            ...
            THEN
               print_debug(gv_module
                           ||':'
                           ||'EXTRACT_DATA:WEB_PAGE Unexpected Error: ' 
                           || sqlerrm, gv_log_type );
               x_retcode   := 1;
               x_err_msg   := gv_module
                              ||':'
       ...
       -- ln_cmt_count := 0;
           ...
           LOOP
              ...
              UPDATE xxaqv_attach_docs_stg xads
               ...
               WHERE 1=1
                 AND xads.datatype_name = 'FILE'
                 AND xads.entity_name   = gv_entity_name; 
            /*IF ln_cmt_count = gn_commit_cnt
            THEN
                 COMMIT;
                 ln_cmt_count := 0;
 ...
 -- This Cursor is used to retrieve information about Short Text Attachments. --
      ...
      IS
       ...
       WHERE fdst.media_id        = fd.media_id
         ...
         AND EXISTS ( SELECT 1
                        FROM xxaqv_ar_inv_lines_c008_stg rcts
                       WHERE rcta.trx_number    = rcts.trx_number )
         AND NOT EXISTS ( SELECT 1 
                            FROM xxaqv_attach_docs_stg xads
                           WHERE xads.trx_number = rcta.trx_number );
 ...
 -- This Cursor is used to retrieve information about Short Text Attachments. --
      ...
      CURSOR lcu_short IS
       ...
       WHERE fdst.media_id    = fd.media_id
         ...
         AND EXISTS ( SELECT 1
                        FROM xxaqv_ar_cust_int_c002_stg acs
                       WHERE acs.customer_number    = ac.customer_number )
         AND NOT EXISTS ( SELECT 1
                            FROM xxaqv_attach_docs_stg xads
                           WHERE xads.customer_number = ac.customer_number);		 
...
-- ****************************************************************************************************************/	
   ...
   BEGIN
      ...
      --
      UPDATE xxaqv_attach_docs_stg
         SET process_flag = gv_load_success
       WHERE process_flag IN ('Validate Error' , 'Import Error')
         AND entity_name  = gv_entity_name;	  
...
-- ****************************************************************************************************************/  
 ...
 TYPE lcu_stg_data_typ IS
         TABLE OF lcu_stg_data%rowtype INDEX BY BINARY_INTEGER;
      lcu_stg_data_tab   lcu_stg_data_typ;
      ln_line_count      BINARY_INTEGER := 0;
      ln_error_count     BINARY_INTEGER := 0;
      ex_dml_errors EXCEPTION;
      PRAGMA exception_init ( ex_dml_errors,-24381 );
...
BEGIN
 ...
 --update stg table with fnd_lobs next value sequence first 
        BEGIN
          UPDATE xxaqv_attach_docs_stg xads
            SET xads.x_file_id      = fnd_lobs_s.NEXTVAL
            WHERE xads.datatype_name  = 'FILE'
              AND xads.entity_name    = gv_entity_name
              AND xads.process_flag   = 'Validate Success';
  
         COMMIT;
        
      EXCEPTION
      WHEN OTHERS THEN
               print_debug( 'INSERTING INTO FND_LOBS: Unexpected Error in update staging : '|| sqlerrm,gv_log_type);
               x_retcode   := 1;
               x_err_msg   := 'INSERTING INTO FND_LOBS: Unexpected Error in update staging :' || to_char(sqlcode) || '-' || sqlerrm;
    END;

   BEGIN
      print_debug('INSERTING INTO FND_LOBS: START INSERTING FILES',gv_log_type);
      print_debug('IMPORT_DATA: pk1_value_from: ' || gv_legacy_id_from);
      print_debug('IMPORT_DATA: pk1_value_to: ' || gv_legacy_id_to);
      --
      x_retcode   := 0;
      x_err_msg   := NULL;
      --
      ...
      LOOP
        BEGIN
         lcu_stg_data_tab.DELETE;
         gt_fnd_lobs_tab.DELETE;
         FETCH lcu_stg_data BULK COLLECT INTO lcu_stg_data_tab LIMIT gn_commit_cnt;
         EXIT WHEN lcu_stg_data_tab.count = 0;
         --
            FOR i IN lcu_stg_data_tab.first..lcu_stg_data_tab.last 
            LOOP
                ln_line_count                                                      := ln_line_count + 1;
                gt_fnd_lobs_tab(ln_line_count).file_id                              := lcu_stg_data_tab(i).x_file_id;
                gt_fnd_lobs_tab(ln_line_count).file_name                            := lcu_stg_data_tab(i).file_name;
                gt_fnd_lobs_tab(ln_line_count).file_content_type                    := lcu_stg_data_tab(i).file_content_type;
                gt_fnd_lobs_tab(ln_line_count).file_data                            := lcu_stg_data_tab(i).file_data;
                gt_fnd_lobs_tab(ln_line_count).upload_date                          := lcu_stg_data_tab(i).upload_date;
                gt_fnd_lobs_tab(ln_line_count).expiration_date                      := lcu_stg_data_tab(i).expiration_date;
                gt_fnd_lobs_tab(ln_line_count).program_name                         := lcu_stg_data_tab(i).program_name;
                gt_fnd_lobs_tab(ln_line_count).program_tag                          := lcu_stg_data_tab(i).program_tag;
                gt_fnd_lobs_tab(ln_line_count).language                             := lcu_stg_data_tab(i).language;
                gt_fnd_lobs_tab(ln_line_count).oracle_charset                       := lcu_stg_data_tab(i).oracle_charset;
                gt_fnd_lobs_tab(ln_line_count).file_format                          := lcu_stg_data_tab(i).file_format;
            END LOOP;

            BEGIN
                FORALL i IN gt_fnd_lobs_tab.first..gt_fnd_lobs_tab.last SAVE EXCEPTIONS
                   INSERT INTO fnd_lobs VALUES gt_fnd_lobs_tab ( i );
            
                print_debug('INSERT FND_LOBS: fnd_lobs: Records loaded sucessfully: ' || SQL%rowcount);
                COMMIT;
                 EXCEPTION
                 WHEN ex_dml_errors THEN
                   x_retcode        := 1;
                   ln_error_count   := SQL%bulk_exceptions.count;
                   print_debug('INSERT FND_LOBS: fnd_lobs: Number of failures: ' || ln_error_count,gv_log_type );

                   FOR i IN 1..ln_error_count 
                    LOOP 
                      print_debug( 'INSET FND_LOBS: fnd_lobs: Error: ' || i || 'Array Index: ' || SQL%bulk_exceptions(i).error_index || 'Message: ' ||
                      sqlerrm(-SQL%bulk_exceptions(i).error_code),gv_log_type);
                    END LOOP;
            
                 WHEN OTHERS THEN
                   print_debug('INSERT FND_LOBS: Unexpected Error: ' || sqlerrm,gv_log_type );
                   x_retcode   := 1;
                   x_err_msg   := 'INSERT FND_LOBS: Unexpected Error:' || to_char(sqlcode) || '-' || sqlerrm;
            END;

         EXCEPTION
         WHEN OTHERS THEN
            print_debug('INSERT FND_LOBS: Unexpected error Bulk Collect:' || sqlerrm,gv_log_type);
        END;
      END LOOP;
      CLOSE lcu_stg_data;
    EXCEPTION
      WHEN OTHERS THEN
               print_debug( 'INSERT FND_LOBS: Unexpected Error : ' || sqlerrm|| ' rowid ' ||lv_curr_record,gv_log_type);
               x_retcode   := 1;
               x_err_msg   := 'INSERT FND_LOBS: Unexpected Error:' || to_char(sqlcode) || '-' || sqlerrm;
    END;
    EXCEPTION
...
-- This Cursor is used to retrieve information from Staging Table --
   ...
   BEGIN
     ...
     EXCEPTION
      ...
      WHEN OTHERS 
      THEN
         print_debug( 'IMPORT: Exception processing Attachments Data:' || sqlerrm, gv_log_type );
         x_retcode   := 1;
...
--      CURSOR lcu_error IS
   ...
   BEGIN
      ...
      print_debug('TIEBACK_STAGING: pk1_value_from: ' || gv_legacy_id_from);
      print_debug('TIEBACK_STAGING: pk1_value_to: ' || gv_legacy_id_to);
          --
      x_retcode   := 0;
      x_err_msg   := NULL;
      --
         ...
         --gt_xxaqv_attach_docs_tab.delete;
          print_debug('TIEBACK_STAGING: Update Error Records');          
               UPDATE xxaqv_attach_docs_stg
                ...
                WHERE 1 = 1
                  AND process_flag         = 'Validate Success'
                  AND entity_name          = gv_entity_name;
    ...
    THEN
        ...
        lv_err_msg := 'Deleting Error Records from Staging table for extract mode';
         ...
         print_debug( lv_err_msg
                    , gv_log_type );
         --
        DELETE FROM xxaqv_attach_docs_stg 
               WHERE entity_name = gv_entity_name 
                 AND process_flag NOT IN ('Load Success','Import Success');
         -- 
